...
多信源天气对比 PWA + 原生 iOS App。覆盖广州市**番禺区**与江西省**安福县**，左右滑动切换城市。Web 端部署在 Cloudflare Pages，番禺气象台数据经阿里云函数计算兜底。

## 功能

- **多信源并排**：逐卡展示各信源实况——温度、体感、湿度、风向风速、观测时间
- **信源可信度评分**：左右滑动卡片评 0–5 分，评分加权影响聚合温度与天气文字；评分持久化到 localStorage；卡片按评分高低自动排序，评分为 0 的信源不计入聚合
- **首屏概览**：加权平均温度 + 天气 · 最高/最低温，全屏天气动效背景
- **关键指标条**：体感、湿度、空气质量、紫外线；体感与湿度始终着色（绿→黄→橙→红），其余偏离正常时按级着色
- **AQI 对比**：多源空气质量逐卡展示，精确到本地监测站
- **气象预警**：当前生效预警信号，按级别配色
- **分钟级降水**：未来一小时降水强度曲线（和风天气）
- **天气动效**：按天气类型 + 昼夜渲染粒子特效，雨/雷/雪走 WebGPU 加速，含真实天文月相与月位
- **CSS Scroll-driven Animations**：Hero 视差 / 指标条渐出 / 温度飞入吸顶条，浏览器原生驱动（Safari 18+、Chrome 115+），不占主线程
- **自动刷新**：下拉刷新，或回前台超 5 分钟 / 每 10 分钟自动刷新；个别信源失败时后台退避补齐
