...
- **天气动效**：`src/WeatherFX.tsx` 用单 canvas + `requestAnimationFrame` 粒子系统，按 `fxKind(天气文字, 昼夜)` 映射到对应特效；切后台暂停、慢动效降帧、resize 重映射粒子坐标。云朵以 SVG turbulence 滤镜生成不规则轮廓，分远/中/近三层景深，日出/日落时受光面自动染暖。
