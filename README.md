...
`ios/` 目录是与 PWA 功能、设计语言对齐的原生版本（SwiftUI + SpriteKit，iOS 17+）。得益于 `URLSession` 无 CORS 限制，**所有数据源均在设备上直接抓取，不依赖任何后端**（番禺核心源开箱即用、无需 key）。构建与详细说明见 [`ios/README.md`](ios/README.md)（`xcodegen generate` 生成工程后用 Xcode 运行）。
