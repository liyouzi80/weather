...
- **下拉刷新 + 自动刷新 + 后台补齐**：下拉手势显示圆弧进度环（随下拉距离填充、松手触发阈值时提亮），触发后过渡为旋转加载环并随内容一起弹簧收起；回到前台超 5 分钟或每 10 分钟自动刷新。刷新完成后若 AQI 仍缺失（服务端抓站点页偶发失败），**后台退避重试并按源合并自动补齐**到界面，不打断当前展示；切城市或再次刷新会让进行中的补齐自动作废。
